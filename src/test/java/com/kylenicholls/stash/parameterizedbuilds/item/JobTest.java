...
public class JobTest {
    ...
    public void testBuildJobTriggers() {
        ...
        List<Trigger> triggers = Stream.of(Trigger.ADD, Trigger.MANUAL)
    ...
    public void testBuildPipelineNotParameterizedAndBuild() {
        String jobName = "jobname";
        Server server = new Server("http://baseurl", null, "", "", false, false);
        Job job = new Job.JobBuilder(0).jobName(jobName).buildParameters("").isPipeline(true)
                .build();
    ...
    public void testBuildUrlParameterizedButScan() {
        ...
        String params = "param1=value1";
        Server server = new Server("http://baseurl", null, "", "", false, false);
        Job job = new Job.JobBuilder(0).jobName(jobName).buildParameters(params).isPipeline(true)
                .build();
        String actual = job.buildUrl(server, bitbucketVariables, false);
    ...
    public void testBuildPipelineParameterizedAndBuild() {
        ...
        assertEquals(server.getBaseUrl() + "/job/" + jobName + "/job/" + branch + 
                "/buildWithParameters?" + params, actual);
    }
    ...
    public void testBuildUrlWithSpecialChars() throws UnsupportedEncodingException {
        ...
        String expectedQuery = "param1=" +  URLEncoder.encode("\"{\"GIT_BRANCH\":\"branchname\"}", 
                "UTF-8");
        assertEquals(server.getBaseUrl() + "/job/" + jobName + "/buildWithParameters?"
